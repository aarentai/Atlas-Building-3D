...
def neumann_conditions_rhs(nabla_TT, g, grad_det_g_x_g_inv, grad_g_inv_x_det_g, det_g_x_g_inv, sqrt_det_g, bdry_idx, bdry_map):
  ...
  for btype, bnum in bdry_map.items():
    ...
    if ("shiftx" in btype) or ("shiftleft" in btype) or ("shiftright" in btype):
      n[0] = 0
      xsign = 0
      #if ("shiftlefttob" in btype) or ("shiftlefttot" in btype) or ("shiftrighttob" in btype) or ("shiftrighttot" in btype):
      #  shiftxtoy = 1
      #else:
      #  shiftxtoz = 1
    elif "left" in btype:
    ...
    if ("shifty" in btype) or ("shiftbottom" in btype) or ("shifttop" in btype):
      n[1] = 0
      ysign = 0
      #if ("shiftbottomtol" in btype) or ("shiftbottomtori" in btype) or ("shifttoptol" in btype) or ("shifttoptor" in btype):
      #  shiftytox = 1
      #else:
      #  shiftytoz = 1
    ...
    if ("shiftz" in btype) or ("shiftrear" in btype) or ("shiftfront" in btype):
      n[2] = 0
      zsign = 0
      #if ("shiftreartol" in btype) or ("shiftreartori" in btype) or ("shiftfronttol" in btype) or ("shiftfronttor" in btype):
      #  shiftztox = 1
      #else:
      #  shiftztoy = 1
    elif "rear" in btype:
    ...
    #                                           + shiftytox * xsign * 7 * det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],1,1] \
    #                                           + shiftztox * xsign * 7 * det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],2,2]) * Bx \
    #                                           +(grad_det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],1,1,1] + grad_g_inv_x_det_g[b_idx[0], b_idx[1],b_idx[2],1,1,1] \
    #                                           + grad_det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],0,0,1] + grad_g_inv_x_det_g[b_idx[0], b_idx[1],b_idx[2],0,1,0] \
    #                                           + grad_det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],2,2,1] + grad_g_inv_x_det_g[b_idx[0], b_idx[1],b_idx[2],2,1,2] \
    #                                           + ysign * 3 * det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],1,1] \
    #                                           + shiftxtoy * ysign * 7 * det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],0,0] \
    #                                           + shiftztoy * ysign * 7 * det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],2,2]) * By \
    #                                           +(grad_det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],2,2,2] + grad_g_inv_x_det_g[b_idx[0], b_idx[1],b_idx[2],2,2,2] \
    #                                           + grad_det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],0,0,2] + grad_g_inv_x_det_g[b_idx[0], b_idx[1],b_idx[2],0,2,0] \
    #                                           + grad_det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],1,1,2] + grad_g_inv_x_det_g[b_idx[0], b_idx[1],b_idx[2],1,2,1] \
    #                                           + zsign * 3 * det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],2,2] \
    #                                           + shiftxtoz * zsign * 7 * det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],0,0] \
    #                                           + shiftytoz * zsign * 7 * det_g_x_g_inv[b_idx[0], b_idx[1],b_idx[2],1,1]) * Bz ) / sqrt_det_g[b_idx[0], b_idx[1],b_idx[2]]
...
# Note, need to fix with args prior to making a LinearOperator
  ...
  for btype, bnum in args['bdry_map'].items():
    ...
    if btype[0:8] == "interior":
      ...
      #     gradYZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradyz_front_idx_3d(alpha, b_idx)
      #   else:
      ...
      #   # if "interiorreartop" in btype:
      #   #   gradYZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradyz_reartopdiag_idx_3d(alpha, b_idx)
      #   # elif "interiorrearbottom" in btype:
      #   #   gradYZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradyz_rearbottomdiag_idx_3d(alpha, b_idx)
      #   # elif "interiorzbottom" in btype:
      #   #   gradYZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradyz_bottom_idx_3d(alpha, b_idx)
      #   # elif "interiorreary" in btype:
      #   #   gradYZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradyz_rear_idx_3d(alpha, b_idx)
      #   # elif "interiorztop" in btype:
      #   #   gradYZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradyz_top_idx_3d(alpha, b_idx)
      #   # elif "interiorfronty" in btype:
      #   #   gradYZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradyz_front_idx_3d(alpha, b_idx)
      #   # else:
      #   #   gradYZ[b_idx[0], b_idx[1],b_idx[2]] = 0
      #   # if "interiorrearright" in btype:
      #   #   gradXZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxz_rearrightdiag_idx_3d(alpha, b_idx)
      #   # elif "interiorrearleft" in btype:
      #   #   gradXZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxz_rearleftdiag_idx_3d(alpha, b_idx)
      #   # elif "interiorzleft" in btype:
      #   #   gradXZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxz_left_idx_3d(alpha, b_idx)
      #   # elif "interiorrearx" in btype:
      #   #   gradXZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxz_rear_idx_3d(alpha, b_idx)
      #   # elif "interiorzright" in btype:
      #   #   gradXZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxz_right_idx_3d(alpha, b_idx)
      #   # elif "interiorfrontx" in btype:
      #   #   gradXZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxz_front_idx_3d(alpha, b_idx)
      #   # else:
      #   #   gradXZ[b_idx[0], b_idx[1],b_idx[2]] = 0
      #   # if "interiorbottomright" in btype:
      #   #   gradXY[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxy_bottomrightdiag_idx_3d(alpha, b_idx)
      #   # elif "interiorbottomleft" in btype:
      #   #   gradXY[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxy_bottomleftdiag_idx_3d(alpha, b_idx)
      #   # elif "interioryleft" in btype:
      #   #   gradXY[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxy_left_idx_3d(alpha, b_idx)
      #   # elif "interiorbottomx" in btype:
      #   #   gradXY[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxy_bottom_idx_3d(alpha, b_idx)
      #   # elif "interioryright" in btype:
      #   #   gradXY[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxy_right_idx_3d(alpha, b_idx)
      #   # elif "interiortopx" in btype:
      #   #   gradXY[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxy_top_idx_3d(alpha, b_idx)
      #   # else:
      #   #   gradXY[b_idx[0], b_idx[1],b_idx[2]] = 0
      ...
      if "shiftx" in btype:
        ...
        if ("shifty" in btype):
          if "tore" in btype:
            xydiff =  (alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1] - alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+1]
                       - alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+1] + alpha[b_idx[0]-1, b_idx[1]-1,b_idx[2]+1])
            xyset+=1
          elif "tof" in btype:
            xydiff =  (alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]-1] - alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]-1]
                       - alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]-1] + alpha[b_idx[0]-1, b_idx[1]-1,b_idx[2]-1])
          ...
          if "tore" in btype:
            xydiff =  (-alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1] + alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+1]
                      + alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]+1] - alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]+1])
            xyset+=1
          elif "tof" in btype:
            xydiff =  (-alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]-1] + alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]-1]
                      + alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]-1] - alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]-1])
        ...
        if ("shiftz" in btype):
          if "tob" in btype:
            xzdiff =  (alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1] - alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+1]
                       - alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]-1] + alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]-1])
            xzset+=1
          elif "tot" in btype:
            xzdiff =  (alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+1] - alpha[b_idx[0]-1, b_idx[1]-1,b_idx[2]+1]
          ...
          if "tob" in btype:
            xzdiff =  (-alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1] + alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+1]
                      + alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+2] - alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+2])
            xzset+=1
          elif "tot" in btype:
            xzdiff =  (-alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+1] + alpha[b_idx[0]-1, b_idx[1]-1,b_idx[2]+1]
                      + alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+2] - alpha[b_idx[0]-1, b_idx[1]-1,b_idx[2]+2])
        ...
        if ("shiftz" in btype):
          if "tob" in btype:
            xzdiff =  (alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]+1] - alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1]
                       - alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]-1] + alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]-1])
            xzset+=1
          elif "tot" in btype:
            xzdiff =  (alpha[b_idx[0]+2, b_idx[1]-1,b_idx[2]+1] - alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+1]
                       - alpha[b_idx[0]+2, b_idx[1]-1,b_idx[2]-1] + alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]-1])
        ...
        if "shifty" in btype:
          ...
          #      - 3.5*alpha[b_idx[0]+1,b_idx[1]+1,b_idx[2]] + 5*alpha[b_idx[0]+2,b_idx[1]+1,b_idx[2]] - 1.5*alpha[b_idx[0]+3,b_idx[1]+1,b_idx[2]]
          #      + 0.5*alpha[b_idx[0]+1,b_idx[1]+2,b_idx[2]] - 0.5*alpha[b_idx[0]+2,b_idx[1]+2,b_idx[2]])
          xydiff = (-0.25*alpha[b_idx[0]+1,b_idx[1],b_idx[2]] + 7.0/12.0*alpha[b_idx[0]-1,b_idx[1],b_idx[2]] - 1.0/3.0*alpha[b_idx[0]+2,b_idx[1],b_idx[2]]
                  - 2.0/3.0*alpha[b_idx[0]-1,b_idx[1]+1,b_idx[2]] + 0.25*alpha[b_idx[0]+1,b_idx[1]+2,b_idx[2]]
                  + 1.0/12.0*alpha[b_idx[0]-1,b_idx[1]+2,b_idx[2]] + 2.0/3.0*alpha[b_idx[0]+2,b_idx[1]+1,b_idx[2]] - 1.0/3.0*alpha[b_idx[0]+2,b_idx[1]+2,b_idx[2]])
          xyset+=1
        elif "shifttop" in btype:
          # TODO figure out this case
          #xydiff = (-3*alpha[b_idx[0]+1,b_idx[1],b_idx[2]] + 4.5*alpha[b_idx[0]+2,b_idx[1],b_idx[2]] - 1.5*alpha[b_idx[0]+3,b_idx[1],b_idx[2]]
        ...
        if "shiftz" in btype:
          ...
          #      - 3.5*alpha[b_idx[0]+1,b_idx[1],b_idx[2]+1] + 5*alpha[b_idx[0]+2,b_idx[1],b_idx[2]+1] - 1.5*alpha[b_idx[0]+3,b_idx[1],b_idx[2]+1]
          #      + 0.5*alpha[b_idx[0]+1,b_idx[1],b_idx[2]+2] - 0.5*alpha[b_idx[0]+2,b_idx[1],b_idx[2]+2])
          xzdiff = (-0.25*alpha[b_idx[0]+1,b_idx[1],b_idx[2]] + 7.0/12.0*alpha[b_idx[0]-1,b_idx[1],b_idx[2]] - 1.0/3.0*alpha[b_idx[0]+2,b_idx[1],b_idx[2]]
                  - 2.0/3.0*alpha[b_idx[0]-1,b_idx[1],b_idx[2]+1] + 0.25*alpha[b_idx[0]+1,b_idx[1],b_idx[2]+2]
                  + 1.0/12.0*alpha[b_idx[0]-1,b_idx[1],b_idx[2]+2] + 2.0/3.0*alpha[b_idx[0]+2,b_idx[1],b_idx[2]+1] - 1.0/3.0*alpha[b_idx[0]+2,b_idx[1],b_idx[2]+2])
          xzset+=1
        elif "shiftfront" in btype:
          # TODO figure out this case
          #xzdiff = (-3*alpha[b_idx[0]+1,b_idx[1],b_idx[2]] + 4.5*alpha[b_idx[0]+2,b_idx[1],b_idx[2]] - 1.5*alpha[b_idx[0]+3,b_idx[1],b_idx[2]]
      ...
      if "shifty" in btype: 
        ...
        if ("shiftz" in btype):
          if "tol" in btype:
            yzdiff =  (alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1] - alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+1]
                       - alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]-1] + alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]-1])
            yzset+=1
          elif "tori" in btype:
            yzdiff =  (alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+1] - alpha[b_idx[0]-1, b_idx[1]-1,b_idx[2]+1]
        ...
        if ("shiftz" in btype) or ("shiftrear" in btype) or ("shiftfront" in btype):
          ...
          #print('in else case 7, btype=', btype)
          ydiff = None # Set to None, because this term is in RHS Neumann condition
        ...
        if ("shiftz" in btype):
          if "tol" in btype:
            yzdiff =  (alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]+1] - alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1]
                       - alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]-1] + alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]-1])
            yzset+=1
          elif "tori" in btype:
            yzdiff =  (alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]+1] - alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+1]
                       - alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]-1] + alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]-1])
        ...
        if "shiftz" in btype:
          ...
          #      - 3.5*alpha[b_idx[0],b_idx[1]+1,b_idx[2]+1] + 5*alpha[b_idx[0],b_idx[1]+2,b_idx[2]+1] - 1.5*alpha[b_idx[0],b_idx[1]+3,b_idx[2]+1]
          #      + 0.5*alpha[b_idx[0],b_idx[1]+1,b_idx[2]+2] - 0.5*alpha[b_idx[0],b_idx[1]+2,b_idx[2]+2])
          yzdiff = (-0.25*alpha[b_idx[0],b_idx[1]+1,b_idx[2]] + 7.0/12.0*alpha[b_idx[0],b_idx[1]-1,b_idx[2]] - 1.0/3.0*alpha[b_idx[0],b_idx[1]+2,b_idx[2]]
                  - 2.0/3.0*alpha[b_idx[0],b_idx[1]-1,b_idx[2]+1] + 0.25*alpha[b_idx[0],b_idx[1]+1,b_idx[2]+2]
                  + 1.0/12.0*alpha[b_idx[0],b_idx[1]-1,b_idx[2]+2] + 2.0/3.0*alpha[b_idx[0],b_idx[1]+2,b_idx[2]+1] - 1.0/3.0*alpha[b_idx[0],b_idx[1]+2,b_idx[2]+2])
          yzset+=1
        elif "shiftfront" in btype:
          # TODO figure out this case
          #yzdiff = (-3*alpha[b_idx[0],b_idx[1]+1,b_idx[2]] + 4.5*alpha[b_idx[0],b_idx[1]+2,b_idx[2]] - 1.5*alpha[b_idx[0],b_idx[1]+3,b_idx[2]]
      ...
      if xxset  < 1 or xxset > 2:
        print('xxdiff not set right amount', xxset, btype)
      if yyset  < 1 or yyset > 2:
        print('yydiff not set right amount', yyset, btype)
      if zzset < 1 or zzset > 2:
        print('zzdiff not set right amount', zzset, btype)
      if xyset < 1 or xyset > 2:
        print('xydiff not set right amount', xyset, btype)
      if xzset < 1 or xzset > 2:
        print('xzdiff not set right amount', xzset, btype)
      if yzset < 1 or yzset > 2:
        #print('yzdiff not set exactly once', yzset, btype)
        print('yzdiff not set right amount', yzset, btype)
...
def solve_3d(in_tens, in_mask, max_iters, clipped_range=None, thresh_ratio=None, save_intermediate_results = False, small_eval = 5e-5, sigma=None):
  ...
  sqrt_gii = np.zeros((xsz,ysz,zsz,3))
  sqrt_gii[:,:,:,0] = np.sqrt(np.abs(g[:,:,:,0,0]))
  sqrt_gii[:,:,:,1] = np.sqrt(np.abs(g[:,:,:,1,1]))
  sqrt_gii[:,:,:,2] = np.sqrt(np.abs(g[:,:,:,2,2]))
  ...
  # says to scale nabla_TT by 1/sqrt(g_ii) for divergence using normalized basis
  #sqrt_det_nabla_TT[:,:,:,0] = sqrt_det_g * nabla_TT[:,:,:,0] / sqrt_gii[:,:,:,0]
  #sqrt_det_nabla_TT[:,:,:,1] = sqrt_det_g * nabla_TT[:,:,:,1] / sqrt_gii[:,:,:,1]
  #sqrt_det_nabla_TT[:,:,:,2] = sqrt_det_g * nabla_TT[:,:,:,2] / sqrt_gii[:,:,:,2]
  if save_intermediate_results:
