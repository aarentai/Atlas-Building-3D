...
def compute_T_derivs_new(TDir, bdry_idx, bdry_map):
  xsz = TDir.shape[0]
  ysz = TDir.shape[1]
  zsz = TDir.shape[2]
  ...
  for btype, bnum in bdry_map.items():
    ...
    if btype == "outside":
      ...
      gradTx_delx[b_idx[0], b_idx[1],b_idx[2]], gradTy_delx[b_idx[0], b_idx[1],b_idx[2]], gradTz_delx[b_idx[0], b_idx[1],b_idx[2]] = xeigvdiff(TDir, b_idx)
      gradTx_dely[b_idx[0], b_idx[1],b_idx[2]], gradTy_dely[b_idx[0], b_idx[1],b_idx[2]], gradTz_dely[b_idx[0], b_idx[1],b_idx[2]] = yeigvdiff(TDir, b_idx)
      gradTx_delz[b_idx[0], b_idx[1],b_idx[2]], gradTy_delz[b_idx[0], b_idx[1],b_idx[2]], gradTz_delz[b_idx[0], b_idx[1],b_idx[2]] = zeigvdiff(TDir, b_idx)
      ...
      if "shifty" in btype:
        if ("shiftx" in btype) or ("shiftleft" in btype) or ("shiftright" in btype):
          ...
          yydiff = (-4*alpha[b_idx[0]+1, b_idx[1],b_idx[2]] + 2*alpha[b_idx[0]+2, b_idx[1],b_idx[2]]
                     + alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]] - alpha[b_idx[0]+2, b_idx[1]-1,b_idx[2]]
        ...
        if "shiftzleft" in btype:
          yzdiff = (0.25*alpha[b_idx[0]+1,b_idx[1]+1,b_idx[2]+1] - 0.25*alpha[b_idx[0]+1,b_idx[1]-1,b_idx[2]+1]
                  - 0.25*alpha[b_idx[0]+1,b_idx[1]+1,b_idx[2]-1] + 0.25*alpha[b_idx[0]+1,b_idx[1]-1,b_idx[2]-1])
        elif "shiftzright" in btype:
          yzdiff = (0.25*alpha[b_idx[0]-1,b_idx[1]+1,b_idx[2]+1] - 0.25*alpha[b_idx[0]-1,b_idx[1]-1,b_idx[2]+1]
                  - 0.25*alpha[b_idx[0]-1,b_idx[1]+1,b_idx[2]-1] + 0.25*alpha[b_idx[0]-1,b_idx[1]-1,b_idx[2]-1])
        elif "shiftrear" in btype:
          # TODO figure out this case
        ...
        if ("shiftx" in btype) or ("shiftleft" in btype) or ("shiftright" in btype):
          ...
          if "bottom" in btype:
            zzdiff = (-4*alpha[b_idx[0], b_idx[1]+1,b_idx[2]] + 2*alpha[b_idx[0], b_idx[1]+2,b_idx[2]]
                       + alpha[b_idx[0], b_idx[1]+1,b_idx[2]-1] - alpha[b_idx[0], b_idx[1]+2,b_idx[2]-1]
          ...
          zzdiff = (-4*alpha[b_idx[0]+1, b_idx[1],b_idx[2]] + 2*alpha[b_idx[0]+2, b_idx[1],b_idx[2]]
                     + alpha[b_idx[0]+1, b_idx[1],b_idx[2]-1] - alpha[b_idx[0]+2, b_idx[1],b_idx[2]-1]
...
def solve_3d(in_tens, in_mask, max_iters, clipped_range=None, thresh_ratio=None, save_intermediate_results = False):
  ...
  gradTx_delx, gradTx_dely, gradTx_delz, gradTy_delx, gradTy_dely, gradTy_delz, gradTz_delx, gradTz_dely, gradTz_delz = compute_T_derivs_new(TDir, bdry_idx, bdry_map)
  ...
  first_nabla_TT[:,:,:,0] = np.multiply(TDir[:,:,:,0], gradTx_delx[:,:,:]) + np.multiply(TDir[:,:,:,1], gradTx_dely[:,:,:]) + np.multiply(TDir[:,:,:,2], gradTx_delz[:,:,:])
  first_nabla_TT[:,:,:,1] = np.multiply(TDir[:,:,:,0], gradTy_delx[:,:,:]) + np.multiply(TDir[:,:,:,1], gradTy_dely[:,:,:]) + np.multiply(TDir[:,:,:,2], gradTy_delz[:,:,:])
  first_nabla_TT[:,:,:,2] = np.multiply(TDir[:,:,:,0], gradTz_delx[:,:,:]) + np.multiply(TDir[:,:,:,1], gradTz_dely[:,:,:]) + np.multiply(TDir[:,:,:,2], gradTz_delz[:,:,:])
  ...
  for k in range(3):
    ...
    second_nabla_TT[:,:,:,k] += christoffel[:,:,:,k,2,0] * TDir[:,:,:,0] * TDir[:,:,:,2]
    second_nabla_TT[:,:,:,k] += christoffel[:,:,:,k,0,2] * TDir[:,:,:,2] * TDir[:,:,:,0]
    second_nabla_TT[:,:,:,k] += christoffel[:,:,:,k,1,1] * TDir[:,:,:,1] * TDir[:,:,:,1]
    second_nabla_TT[:,:,:,k] += christoffel[:,:,:,k,2,1] * TDir[:,:,:,1] * TDir[:,:,:,2]
    second_nabla_TT[:,:,:,k] += christoffel[:,:,:,k,1,2] * TDir[:,:,:,2] * TDir[:,:,:,1]
    second_nabla_TT[:,:,:,k] += christoffel[:,:,:,k,2,2] * TDir[:,:,:,2] * TDir[:,:,:,2]
