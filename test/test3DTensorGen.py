...
def testAnnulusTens3D(xsz, ysz, out_prefix):
  circ_rng = (-1.4,1.4)
  img, tens, seed, xrg, yrg, zrg, zsz = data.gen.gen_3D_annulus(xsz, ysz, 6.0, True, False, False, xrng = circ_rng, yrng = circ_rng, zrng=None)
...
def testCubic1Tens3D(xsz, ysz, out_prefix, xrg=None, yrg=None, zrg=None, zero_padding_width=None):
  ...
  return(testGenAndWriteCubic3D(xsz, ysz, a0, a1, a2, a3, b0, b1, b2, b3, out_prefix, xrg, yrg, zrg, zero_padding_width))
...
def testCubic2Tens3D(xsz, ysz, out_prefix, xrg = None, yrg = None, zrg=None, zero_padding_width=None):
  ...
  return(testGenAndWriteCubic3D(xsz, ysz, a0_2, a1_2, a2_2, a3_2, b0_2, b1_2, b2_2, b3_2, out_prefix, xrg, yrg, zrg, zero_padding_width))

def testGenAndWriteCubic3D(xsz, ysz, a0, a1, a2, a3, b0, b1, b2, b3, out_prefix, xrg=None, yrg=None, zrg=None, zero_padding_width=None):
  c1 = lambda t: data.gen.cubic(t, a3, a2, a1, a0)
  dc1 = lambda t, dt: data.gen.d_cubic(t, a3, a2, a1, a0, dt)
  c2 = lambda t: data.gen.cubic(t, b3, b2, b1, b0)
  dc2 = lambda t, dt: data.gen.d_cubic(t, b3, b2, b1, b0, dt)
  # TODO ok if xrg, yrg not isotropic?
  (cubic_no_blur_img, cubic_no_blur_ten, cubic_no_blur_seed, cubic_no_blur_xrg, cubic_no_blur_yrg, cubic_no_blur_zrg, zsz) = data.gen.gen_3D_tensor_image(xsz, ysz, 0, 1, 1000, c1, dc1, c2, dc2, 1/5.0, 15, 6.0, 0.05, 0.95,True,False,False, xrg, yrg, zrg, zero_padding_width=zero_padding_width)
...
def testRandomCubicTens3D(xsz, ysz, out_prefix, num_cubics=None, xrg=None, yrg=None, zrg=None, zero_padding_width=None):
  a3 = -4 #-4.0
  a2 = 4 #2.0
  a1 = 2 #10.0
  a0 = 0.0 # 0.0
  ...
  if num_cubics is None:
    num_cubics = 2
  if num_cubics >= 1:
    xrg, yrg, zrg = testGenAndWriteCubic3D(xsz, ysz, a0, a1, a2, a3, b0, b1, b2, b3, out_prefix + "cubic1_", xrg, yrg, zrg, zero_padding_width)
  else:
    return

  if num_cubics >= 2:
    testGenAndWriteCubic3D(xsz, ysz, a0_2, a1_2, a2_2, a3_2, b0_2, b1_2, b2_2, b3_2, out_prefix + "cubic2_", xrg, yrg, zrg, zero_padding_width)
  else:
    return
  ...
  for cc in range(3,num_cubics+1):
    ...
    #b0_r = 0.0
    testGenAndWriteCubic3D(xsz, ysz, a0_r, a1_r, a2_r, a3_r, b0_r, b1_r, b2_r, b3_r, out_prefix + f"cubic{cc}_" , xrg, yrg, zrg, zero_padding_width)
  return(xrg, yrg, zrg)
...
"""
  ...
  testAnnulusTens3D(int(options.xsz), int(options.ysz), options.outdir + "/metpy_annulus_3D_")
  cubic_xrg = [-0.4, 2.6]
  cubic_yrg =[-0.4, 2.6]
  cubic_xrg = [-1.25, 8.4]
  cubic_yrg =[-1.25, 8.4]
  ...
  testRandomCubicTens3D(int(options.xsz), int(options.ysz), options.outdir + "/metpy_3D_", int(options.numcubics), cubic_xrg, cubic_yrg, cubic_zrg, 4)
